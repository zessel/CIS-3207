...
void split_background_arguments(int argsnum, char *argument_array[], char *right_argument_array[]);
...
void main (int argc, char *argv[])
{
    ...
    */
        ...
        { 
            ...
            char *right_argument_array[argsnum];


     //       while(check_args_for(argsnum, argument_array,"&") > 1)
      //      {
       //         split_background_arguments(argsnum, argument_array, right_argument_array);
        //        argsnum = recount_args(argument_array);
         //   }
...
void split_background_arguments(int argsnum, char *argument_array[], char *left_argument_array[])
{
    int hit_ampersand = 0;    
    char *temp[argsnum];
    ...
    {
        temp[i] = argument_array[i];
    }

    for (int i = 0; i < argsnum; i++)
    {
        if (hit_ampersand == 0)
            left_argument_array[i] = argument_array[i];
        if (hit_ampersand != 0)
        {
            argument_array[i-hit_ampersand-1] = argument_array[i];
        }
    ...
    left_argument_array[argsnum-hit_ampersand+1] = NULL;
    argument_array[argsnum-recount_args(left_argument_array)] = NULL;

    int pid;
    if ((pid = fork()) == 0)
    {
        execute_command(recount_args(left_argument_array),left_argument_array);
        quit();
    }
}
...
void reset_output(int old_output_fd)
{
        fprintf(stderr,"reseting output");
...
int redirect_output(char *filename, int append)
{
        fprintf(stderr,"redirecting otuput");
...
void run_external_function(int argsnum, char *argument_array[])
{
    ...
    if (pid == 0)
    {
        fprintf(stdout, "in the right thing");
        set_env_parent();
